...
export default function ThemeToggle() {
  ...
  // Styled button with primary colors. Icon indicates current mode (moon = dark, sun = light)
  return (
    <button
      type="button"
      onClick={toggle}
    ...
    >
      ...
      {isDark ? (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-4 h-4 text-primary-light">
      ...
      <span className="text-sm">
        {isDark === undefined ? '' : isDark ? 'Light Mode' : 'Dark Mode'}
      </span>
    </button>
  );
}
